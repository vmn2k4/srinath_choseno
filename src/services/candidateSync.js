...
const CA_PROVINCIAL_PROPERTY_SIGNATURE = {
  ed_abbreviation: 'bc',
  ednameen: 'mb',
  ed_id: 'on',
  nm_cep: 'qc',
...
const JURISDICTIONS_WITH_FETCH = new Set([
...
export function detectJurisdiction(mapShape, roleTitle) {
...
// keep these in sync.
async function getCurrentEventsByJurisdiction() {
  const [{ data: fed }, { data: prov }] = await Promise.all([
    ...
    supabase.from('federal_election_events').select('*').order('is_general', { ascending: false }).order('event_date', { ascending: false, nullsFirst: false }),
    supabase
  ...
  const byProvince = {};
  (prov || []).forEach(e => { if (!byProvince[e.province]) byProvince[e.province] = e; });

  return {
    'ca-federal': fed?.[0] || null,
...
function buildCandidateUrl(mapShape, jurisdiction, event) {
...
export async function getCandidateSourceInfoForSeats(seats) {
  const events = await getCurrentEventsByJurisdiction();
  const result = new Map();
  for (const seat of seats) {
    const mapShape = seat.map_shapes;
    const jurisdiction = detectJurisdiction(mapShape, seat.role_title);
    if (!jurisdiction) {
      result.set(seat.id, { jurisdiction: null, status: 'unsupported', url: null, label: 'No official source built for this seat type yet' });
      continue;
    }
    const status = (url) => (JURISDICTIONS_WITH_FETCH.has(jurisdiction) ? 'active' : (url ? 'manual_only' : 'unsupported'));
    ...
    if (jurisdiction === 'us-federal' || jurisdiction === 'us-senate') {
      ...
      const state = jurisdiction === 'us-federal'
        ? US_STATE_FIPS[mapShape.properties?.statefp]
      ...
      const url = `https://www.fec.gov/data/candidates/?${new URLSearchParams({ cycle: String(cycle), office, state, ...(district ? { district } : {}) }).toString()}`;
    ...
    if (usStateMatch) {
      ...
      if (!entry) {
        result.set(seat.id, { jurisdiction, status: 'unsupported', url: null, label: 'No official source built for this seat type yet' });
        continue;
      }
    ...
    if (!event) {
      result.set(seat.id, { jurisdiction, status: 'no_event', url: null, label: `No known ${jurisdictionLabel(jurisdiction)} election event yet` });
      continue;
    }
    const url = buildCandidateUrl(mapShape, jurisdiction, event);
    result.set(seat.id, { jurisdiction, status: status(url), url, label: event.name, eventId: event.id });
  }
  return result;
}
...
export async function fetchOfficialCandidates(seatId) {
...
export async function addFetchedCandidate(seatId, country, candidate) {
  let partyId = null;
  if (candidate.party && country) {
    const { data: parties } = await getPoliticalParties({ country });
    const needle = candidate.party.trim().toLowerCase();
    const match = (parties || []).find(p =>
      p.name.toLowerCase() === needle || p.name.toLowerCase().includes(needle) || needle.includes(p.name.toLowerCase())
    );
    partyId = match?.id ?? null;
  }
  return addUnregisteredCandidate(seatId, {
