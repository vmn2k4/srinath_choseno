...
interface CandidateRow {
  ...
  profiles?: {
    ...
    politician_profiles?: { avatar_url?: string | null } | { avatar_url?: string | null }[];
  } | null;
}
  ...
  return (
    <Card padding="md" className="space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-text-muted mt-1 max-w-md">
            {leader
              ? `${leader.name} is currently leading with ${totalSupport > 0 ? Math.round((leader.supporterCount / totalSupport) * 1000) / 10 : 0}% community support.`
      ...
      <div className="space-y-3">
        {rows.map(({ candidate, name, avatarUrl, supporterCount }) => {
              ...
              <div className="h-2.5 w-full rounded-full bg-surface-active overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    isLeader && totalSupport > 0 ? "bg-primary" : "bg-primary/40"
                  }`}
                  style={{ width: `${pct}%` }}
                />
              </div>
