...
export default function AdminNewsDistributionClient() {
  ...
  const fetchArticles = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setStatusMessage({ text: err?.message || "Failed to load news articles", type: "error" });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    return articles
      .filter((art) => {
        ...
        if (searchQuery.trim()) {
          ...
          if (!matchHeadline && !matchSlug && !matchTweet && !matchPolitician && !matchBatch) {
            return false;
          }
        }
  ...
  }, [articles, selectedBatch, searchQuery, sortBy]);
  ...
  const handleExportCsv = () => {
    ...
    showToast(`Exported ${filteredArticles.length} rows to CSV`, "success");
  };
    ...
    <div className="min-h-screen bg-background text-text-main p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-[1600px] mx-auto space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-surface p-3.5 rounded-xl border border-border">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                <option value="all">All Batches ({articles.length})</option>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 bg-background border border-border rounded-lg text-xs text-text-main placeholder:text-text-muted w-48 sm:w-64 focus:outline-none focus:border-primary"
              />
            </div>
        ...
        <div className="bg-surface rounded-xl border border-border overflow-x-auto shadow-sm">
          {loading ? (
            <div className="p-12 text-center text-xs text-text-muted">Loading published articles...</div>
          ) : filteredArticles.length === 0 ? (
            <div className="p-12 text-center text-xs text-text-muted">No published articles found.</div>
          ) : (
