...
export async function deleteBoundaryType(supabase: Client, typeId: string) {
  invalidateCache("boundary_types");
...
export async function getMapShapeIdsByUploadId(supabase: Client, uploadId: string) {
  return fetchAllPages((from, to) =>
    supabase
      .from("map_shapes")
      .select("id")
...
export async function getMapShapeCodesByUploadId(supabase: Client, uploadId: string) {
...
export async function renameBoundaryUpload(supabase: Client, uploadId: string, name: string) {
...
export async function finalizeBoundaryUpload(supabase: Client, uploadId: string, completedAt: string) {
  return supabase.from("boundary_uploads").update({ completed_at: completedAt }).eq("id", uploadId);
}

export async function deleteBoundaryUpload(supabase: Client, uploadId: string) {
  return supabase.rpc("delete_boundary_upload", { p_upload_id: uploadId });
}

// ── redistricting rpcs ───────────────────────────────────────────────────
export async function suggestReplacedShapes(supabase: Client, uploadId: string) {
  return fetchAllPages((from, to) =>
    supabase.rpc("suggest_replaced_shapes", { p_upload_id: uploadId }).range(from, to)
  );
}
