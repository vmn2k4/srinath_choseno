...
async function forceReformat() {
  console.log('Force reformatting top 30 articles in Supabase...');
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    if (updateRes.ok) {
      console.log(`[${i+1}/30] Successfully updated: ${art.slug}`);
    } else {
      console.error(`Failed to update ${art.slug}:`, await updateRes.text());
    }
  }

  console.log('\nAll 30 articles successfully updated in database!');
}
