...
// Exported so rss-verified-pipeline.js can call this directly for
// on-demand/auto-window runs instead of shelling out and re-parsing stdout.
async function getLastPublishWindow() {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch latest article: ${await res.text()}`);
  }
...
if (require.main === module) {
  getLastPublishWindow().then(result => {
...
module.exports = { getLastPublishWindow };
