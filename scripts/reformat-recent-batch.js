...
async function reformatRecent20() {
  console.log('Fetching top 30 recent articles to apply the Google Reviews format...');
  
  const res = await fetch(`${SUPABASE_URL}/rest/v1/news_articles?select=id,slug,headline,summary,province,country,category,content,news_article_politicians(politician_id, profiles(id, full_name, politician_profiles(wall_slug)))&order=created_at.desc&limit=30`, {
  ...
  for (const art of articles) {
    ...
    if (updateRes.ok) {
      console.log(`Updated [${updatedCount+1}]: ${art.slug}`);
      updatedCount++;
    }
  }

  console.log(`\nSuccessfully reformatted ${updatedCount} recent articles!`);
}
