/**
 * ⚠️  DEPRECATED — DO NOT RUN OR REUSE AS A TEMPLATE. ⚠️
 *
 * This script does NOT produce genuine journalism. `buildArticleObject()`
 * below synthesizes article `body` text from a fixed generic template
 * ("State and federal leaders have enacted key policy measures under the
 * {category} portfolio..."), fabricates a fake source URL per story
 * (`https://news.gov.ca/releases/[slug]` — this domain does not exist and
 * was never a real government press release), and the "expanded" 40 CA /
 * 50 US stories below that are generated by `for` loops fabricate dollar
 * figures out of loop index arithmetic (e.g. `i * 15 + 75`), hardcode the
 * WRONG politician for most provinces/states, and reuse one hardcoded
 * lat/lng pair (Ottawa or Washington DC) for every story regardless of
 * where it's supposed to have happened.
 *
 * This is exactly the "templated batch placeholders" / fabricated-content
 * problem that commits 5fa2323 and 8d6df61 (see `git log`) had to clean up
 * after this script was run once. It was left in the repo unfixed and is a
 * hazard if reused as a "100-story batch" template in a future session.
 *
 * Use `scripts/insert-news-batch.js` instead — it takes fully hand-authored
 * article objects (real body, real verified source URLs, real per-story
 * lat/lng, real linked_profile_id lookups) and scales to 100 entries in its
 * `articles` array exactly the same way. See NewsCollectionPrompt.md.
 *
 * `run()` below refuses to execute so this can't be invoked by accident.
 */

if (require.main === module) {
  console.error(
    '\nscripts/publish-100-stories-batch.js is DEPRECATED and refuses to run.\n' +
    'It fabricates article body text, source URLs, dollar figures, and geo-coordinates.\n' +
    'Use scripts/insert-news-batch.js instead (see file header + NewsCollectionPrompt.md).\n'
  );
  process.exit(1);
}
