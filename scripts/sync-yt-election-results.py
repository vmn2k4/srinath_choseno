#!/usr/bin/env python3
"""
Yukon Municipal Sync — Mayors & Councillors

Source of truth for Yukon: the Government of Yukon's own official "Local
government directory" PDF (yukon.ca), updated monthly -- covers all 8
incorporated Yukon municipalities in a clean 3-column table (address |
Mayor + Councillors | CAO + meeting schedule) via pdfplumber's
extract_table(), the same reliable approach used for Newfoundland and
Labrador this session (see OFFICE_HOLDERS_DATA_GUIDE.md for why that's
preferred over word-position heuristics after the Manitoba incident).

Yukon had zero current officeholders before this -- OpenNorth doesn't
cover the territory at all.

Local advisory areas (Hamlet of Ibex Valley, Hamlet of Mount Lorne, Marsh
Lake/South Klondike/Tagish Local Advisory Councils) are a different
category -- elected advisory councils, not incorporated municipalities --
and not imported here; office_holders has no role type for them yet.

Matching is scoped to Yukon via shape_containers throughout -- see
OFFICE_HOLDERS_DATA_GUIDE.md for why an unscoped name match is not safe.

Usage: python3 scripts/sync-yt-election-results.py [--apply]
"""
...
PDF_URL = "https://yukon.ca/sites/default/files/cs-local-government-directory-2026-06-16_.pdf"
SOURCE_URL = "https://yukon.ca/en/local-government-directory"
...
def normalize_municipal_name(s):
    if not s:
        return ""
    s = s.lower()
    s = re.sub(r"^(the village of|city of|town of|village of)\s+", "", s)
    s = re.sub(r"[^a-z0-9]+", " ", s)
    return s.strip()
        ...
        for page in pdf.pages:
            for table in page.extract_tables():
                for row in table:
                    ...
                    if not m:
                        continue
                    mayor = m.group(1).strip()
                    councillors = [c.strip() for c in m.group(2).split("\n") if c.strip()]
                    munis[name] = {"mayor": mayor, "councillors": councillors}
    return munis
...
def main():
    ...
    print("Fetching Yukon's Local Government Directory PDF...")
    r = requests.get(PDF_URL, impersonate="chrome")
    pdf_path = "/tmp/yt_directory.pdf"
    with open(pdf_path, "wb") as f:
        f.write(r.content)

    munis = parse_pdf(pdf_path)
    print(f"Parsed {len(munis)} municipalities: {list(munis.keys())}")

    print("Loading map_shapes (Municipal, Yukon only, active)...")
    shapes = run_sql("""
        SELECT ms.id, ms.name FROM map_shapes ms
        JOIN shape_containers sc ON sc.map_shape_id = ms.id
        JOIN map_shapes p ON p.id = sc.container_shape_id AND p.name = 'Yukon'
    ...
    for muni, data in munis.items():
        ...
        if not shape_id:
            ...
            continue
        winners = [(MAYOR_ROLE_ID, data["mayor"])] + [(COUNCILLOR_ROLE_ID, c) for c in data["councillors"]]
        winners_by_shape[shape_id] = winners

    total_winners = sum(len(v) for v in winners_by_shape.values())
    print(f"Matched {total_winners} officeholders across {len(winners_by_shape)} Yukon municipalities.")
    if unmatched_munis:
        print(f"Unmatched (not silently dropped): {unmatched_munis}")
    if dupes:
        print(f"map_shapes has duplicate names for {len(dupes)} Yukon municipalities -- excluded entirely rather than guessed at: {sorted(dupes)}")
...
""")
    ...
    sql.append("INSERT INTO staging_yt_winners VALUES\n" + ",\n".join(value_rows) + ";")
...
""")
    ...
    out_path = "/Users/vmn2k4/Coding/Choseno/scripts/yt-election-results-sync.sql"
