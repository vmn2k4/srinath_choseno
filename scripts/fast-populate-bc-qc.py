#!/usr/bin/env python3
"""
Fast Batched Ingestion for BC & Quebec Municipal Parties
"""
...
def main():
    print("🚀 Running fast batched party ingestion...")
    ...
    # 3. Build single SQL transaction batch
    sql_statements = ["BEGIN;"]
    
    for p_name in sorted(all_parties):
        safe_p = p_name.replace("'", "''")
        sql_statements.append(f"INSERT INTO public.political_parties (country, name, rank) VALUES ('Canada', '{safe_p}', 100) ON CONFLICT (country, name) DO NOTHING;")

    sql_statements.append("COMMIT;")
    
    # Execute party seeding batch
    psql_cmd = ["psql", DB_URI, "-c", "\n".join(sql_statements)]
    subprocess.run(psql_cmd, check=True)

    # 4. Fetch DB party IDs
    psql_get_parties = ["psql", DB_URI, "-t", "-A", "-F", "\t", "-c", "SELECT id, name FROM public.political_parties WHERE country = 'Canada';"]
    party_rows = subprocess.run(psql_get_parties, capture_output=True, text=True).stdout.strip().split("\n")
    party_map = {r.split("\t")[1].lower(): int(r.split("\t")[0]) for r in party_rows if "\t" in r}

    # 5. Fetch DB office holders
    psql_get_oh = ["psql", DB_URI, "-t", "-A", "-F", "\t", "-c", """
    ...
    """]
    oh_rows = subprocess.run(psql_get_oh, capture_output=True, text=True).stdout.strip().split("\n")

    update_statements = ["BEGIN;"]
    updated_count = 0

    for r in oh_rows:
        parts = r.split("\t")
        if len(parts) < 3: continue
        oh_id, full_name, city_name = parts[0], parts[1], parts[2]
        ...
        if matched_party:
            ...
            if party_id:
                update_statements.append(f"UPDATE public.office_holders SET political_party_id = {party_id} WHERE id = '{oh_id}';")
                updated_count += 1

    update_statements.append("COMMIT;")
    
    psql_update_cmd = ["psql", DB_URI, "-c", "\n".join(update_statements)]
    subprocess.run(psql_update_cmd, check=True)

    print(f"🎉 Fast update completed! Updated {updated_count} office holders.")

    # 6. Re-export office-holders-data.csv
    csv_rows_raw = subprocess.run([
        "psql", DB_URI, "-t", "-A", "-F", "\t", "-c", """
