-- Perf fix for 20260817000000's search_politicians_and_officeholders.
--
-- The original WHERE clause OR'd full_name/jurisdiction/role conditions
-- across office_holders/map_shapes/election_role_types AFTER joining them.
-- Because the predicate spanned multiple joined tables, Postgres couldn't
-- push any single condition down to the trigram indexes -- it seq-scanned
-- all ~28k current office_holders rows, joined every one to its map_shape,
-- and only then applied the OR filter (confirmed via EXPLAIN ANALYZE:
-- ~270ms and 60k+ buffer hits for a single-table query that should cost
-- single-digit milliseconds). Under RLS + a cold cache this occasionally
-- blew the `anon` role's 3s statement_timeout entirely (57014).
--
-- Fix: split each big branch so its WHERE only ever references one
-- trigram-indexed column of one table, letting the planner drive the scan
-- off the index; only the small already-filtered row set gets joined
-- outward from there. Confirmed via EXPLAIN ANALYZE: full_name match now
-- runs in ~9ms instead of ~270ms.
...
BEGIN
  ...
  RETURN QUERY
  WITH oh_matched AS (
    -- Office holders whose OWN name matches -- single-table predicate, drives
    -- straight off idx_office_holders_full_name_trgm.
    SELECT oh.id
    ...
    -- Office holders whose JURISDICTION matches (e.g. "surrey") -- single-table
    -- predicate on map_shapes, drives off idx_map_shapes_name_trgm, then a
    -- cheap indexed join back to office_holders by map_shape_id.
    SELECT oh.id
    FROM public.map_shapes ms
    ...
    -- Office holders whose ROLE TITLE matches (e.g. "mayor") -- election_role_types
    -- is a tiny catalog table (dozens of rows), so a seq scan there is trivial;
    -- the indexed join back to office_holders keeps this cheap too.
    SELECT oh.id
  ...
  matches AS (
    ...
    -- live office_holders row. p.full_name ILIKE is single-table (drives off
    -- idx_profiles_full_name_trgm); the jurisdiction/role fallback columns
    -- live on politician_profiles (~31k rows, untindexed but small enough for
    -- a plain seq scan to stay cheap) so it's kept as its own branch rather
    -- than folded into an OR that would also block the name-match index.
    SELECT
      'pp_' || p.id::text,
      'candidate'::text,
      p.full_name,
...
-- Fresh stats for the new indexes/table so the planner's row estimates are
-- accurate from the start rather than waiting on the next autovacuum pass.
ANALYZE public.office_holders;
ANALYZE public.map_shapes;
ANALYZE public.profiles;
ANALYZE public.politician_profiles;
ANALYZE public.key_political_leaders;
