-- "One result per person" -- David Eby (Premier + MLA for his home riding,
-- both correctly linked to the same profile since the officeholder merge)
-- was still showing as two separate search results, even though both now
-- resolve to the exact same wall. Once two office_holders rows share a
-- politician_profile_id, they represent one search result, not two.
--
-- DISTINCT ON collapses to one row per distinct profile (falling back to
-- each row's own result_key when politician_profile_id is null -- an
-- unlinked office_holder or a candidate with no profile is never merged
-- with anything else, only genuinely-the-same-profile rows are). The pick
-- within a group uses the same ranking the final ordering already applies,
-- so the more senior/closer-matching role naturally wins the display slot;
-- role_title is added as a final deterministic tiebreak. A second pass
-- re-sorts the deduped set globally and applies the limit, since DISTINCT
-- ON's own ORDER BY has to lead with the grouping key.
...
BEGIN
  ...
  deduped AS (
    -- Collapse every row sharing a real politician_profile_id (multiple
    -- offices held by the same merged profile) down to just its best match;
    -- rows with no profile link stay one-per-row via the result_key fallback.
    SELECT DISTINCT ON (COALESCE(r.politician_profile_id::text, r.result_key))
      ...
      r.match_rank, r.proximity_tier, r.locality_scope
    FROM ranked r
    ORDER BY
      COALESCE(r.politician_profile_id::text, r.result_key),
  ...
  SELECT
    d.result_key, d.source, d.full_name, d.role_title, d.jurisdiction_name,
    d.country, d.boundary_type, d.map_shape_id, d.party_name, d.photo_url,
    d.wall_slug, d.politician_profile_id, d.office_holder_id,
    d.is_key_leader, d.key_priority_raw
  FROM deduped d
  ORDER BY
    d.is_key_leader DESC,
    COALESCE(d.key_priority_raw, 9) ASC,
    d.match_rank ASC,
    d.proximity_tier ASC,
    d.locality_scope ASC,
    d.full_name ASC
  LIMIT p_limit;
END;
$$;
