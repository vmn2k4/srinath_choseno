// JSX layout for the news-article share card, split into its own .tsx file
// so index.ts (the Deno.serve entrypoint) never needs a JSX-capable parse --
// same split as supabase/functions/generate-election-og-image/card.tsx and
// Supabase's own "Generating OG Images" Edge Function example
// (handler.tsx + index.ts). Rendered via @vercel/og's ImageResponse (Satori
// + resvg-wasm under the hood), same engine src/lib/utils/og.tsx already
// uses on the Vercel side -- just invoked from Deno here instead of
// Next.js.
//
// This is a straight port of buildNewsArticleOgCardElement in
// src/lib/utils/ogCard.tsx -- not imported from there directly, because that
// file is bundled by Next.js/webpack and assumes Node-ish resolution, which
// doesn't carry over to a plain Deno import graph (same reason
// generate-election-og-image/card.tsx doesn't import ogCard.tsx either).
// Keep this in sync by eye if ogCard.tsx's news card layout changes.
...
// ../generate-profile-og-image/card.tsx, which this now deliberately
// mirrors (same banner copy, same dark ground, same star-rating language)
// instead of v1's light pastel-gradient "civic dashboard" look. Full-bleed
...
// banner, then the article's actual headline and its takeaways. Kept in
// sync by eye with buildNewsArticleOgCardElement in src/lib/utils/ogCard.tsx
// -- see this file's header comment for why it's not imported directly.
export function NewsArticleOgCard(input: NewsArticleOgCardInput) {
