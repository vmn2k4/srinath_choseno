...
| `politicianWall.ts` | politician_supporters + anonymous_supporters (logged-out support, added 2026-09-10 — see `add_anonymous_support`/`remove_anonymous_support` RPCs and the admin breakdown RPC), wall-scoped profile/posts lookups, wall post creation (direct insert), realtime support-count subscription | `PoliticianWallClient`, `CandidacyWall`, `ElectionSeatPageClient`, `ModerationPageClient` (admin breakdown) |
...
| `settings.ts` | `site_settings` (single-row config: theme, platform rate-limit knobs, anonymous-support kill switch + rate-limit cap), cached via `src/lib/utils/apiCache.ts` | `ThemeContext`, `ModerationPageClient` |
