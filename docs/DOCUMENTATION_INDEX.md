...
- **[ELECTION_DATA_SOURCES.md](ELECTION_DATA_SOURCES.md)** — Where candidate data comes from, how verified candidates are listed. Now includes BC's verified municipal candidate source (Elections BC's LECFA registered-candidates PDF), the live status of Ontario/Manitoba Municipal elections and US Governor seats, and a real boundary-classification bug found while scoping Saskatchewan/PEI/NWT (see its "Lessons learned" §12)
- **[adding-us-2026-midterm-candidates.md](adding-us-2026-midterm-candidates.md)** — How the 2026 US Midterms election + its House/Senate races + confirmed candidates were bulk-loaded from the FEC; repeatable recipe for future cycles. Governor seats (36 states) were added onto this same election 2026-09-03 — note `start_us_2026_midterms.py` cannot source Governor candidates, the FEC has none
- **[CANDIDATE_DATA_PULL_LOG.md](CANDIDATE_DATA_PULL_LOG.md)** — The actual pull, election by election: what source each open election's real candidates came from, what got skipped and why, and the new dropped-out-candidate detection (`scripts/refresh_us_2026_candidates.py`) alongside the existing add-only scripts
...
| Date | Added | Author |
|---|---|---|
| 2026-09-04 | Follow-up to the primary-contamination cleanup: a user-requested 10-race spot-check via live web search found 4/10 races missing a genuine minor-party/independent candidate (Wikipedia's infobox only shows headline nominees). Re-swept all 44 fixed states against Ballotpedia's full certified-ballot pages instead — ~350 minor-party/independent/write-in candidates added, 31 new minor parties created, 3 wrongly-deleted independents restored (OR-06, MS-04, NE-03/MT-02), 2 false Wikipedia-sourced additions corrected (WY, ND), and 6 California districts' second-place candidate corrected after a Statement-of-Vote PDF misread. New reusable `scripts/us_house_primary_fixes/add_missing.py`. Full writeup in CANDIDATE_DATA_PULL_LOG.md | Claude |
| 2026-09-04 | Finished the US House primary-contamination cleanup started 2026-09-03: fixed the remaining ~43 states (Ohio through Alaska/Vermont) using Wikipedia's per-state election articles + a new streamlined `scripts/us_house_primary_fixes/apply_fix.py`, all 47 per-state truth files saved to `scripts/us_house_primary_fixes/state_truth_files/`. Handled Louisiana's all-party jungle primary (no elimination yet — kept every declared candidate), Alaska's top-4 ranked-choice advance, Utah's mid-cycle court-ordered redistricting, and 4 same-surname false-positive matches caught by a final national verification sweep (IL-04, MI-11, TX-22, TX-34) plus one duplicate-profile cleanup (NJ-08). North Carolina (uncertified) and New Hampshire/Rhode Island (primaries not yet held as of this date) deliberately left unfixed. Full writeup in CANDIDATE_DATA_PULL_LOG.md | Claude |
| 2026-09-03 | Found and started fixing a systemic bug: FEC's `candidate_status` has zero concept of state primary elimination, so 341 of 435 US House districts (78%) were showing eliminated primary losers as active candidates, sometimes missing the real nominee entirely (surfaced by a user checking their own district live). Confirmed Google's Civic Information/VIP API (tested with a newly-created, properly-scoped key) is real and official but its candidate-roster data isn't populated yet, even for elections days away. Fixed WA-02, all of South Dakota (House+Senate), and all 8 Maryland House districts by hand using each state's official post-primary certified results — CANDIDATE_DATA_PULL_LOG.md has the full writeup and remaining scope (334 districts left) | Claude |
| 2026-09-03 | Second pass at the 15 remaining Governor states from the same day's Batch 2: actually fetched every one to pin down the exact blocker (7 hard-blocked 403, 3 need a real browser session, Kansas confirmed to have no 2026 general list published yet, Ohio's whole site is down for maintenance, 3 had wrong guessed URLs, Vermont's real export filename still not found, New Mexico needs a filer login) — zero new candidates converted this round, but ELECTION_DATA_SOURCES.md and CANDIDATE_DATA_PULL_LOG.md now carry precise per-state reasons instead of "not yet tried" | Claude |
| 2026-09-03 | Researched real 2026 Governor candidate sources for all 25 remaining states beyond Batch 1 (ELECTION_DATA_SOURCES.md's new "Batch 2" section) — 2 more states (Maryland, South Dakota) verified and added to Choseno (32 of 36 Governor seats now have real candidates), the rest split honestly across bot-blocked / needs-session-scraping / found-but-not-yet-fetched / primary-not-decided-yet. CANDIDATE_DATA_PULL_LOG.md updated with the final per-state breakdown | Claude |
| 2026-09-03 | New CANDIDATE_DATA_PULL_LOG.md (the actual first candidate pull for every open election: 65 real BC candidates via LECFA with officeholder-dedup, 26 US Governor candidates via 3 states' live-fetch handlers, US House/Senate refreshed against live FEC data with new dropped-out-candidate detection — `scripts/refresh_us_2026_candidates.py` and `scripts/add_governor_candidates.py`, both new; Ontario/Manitoba confirmed to have no centralized candidate source after a real search, not just unresearched) | Claude |
| 2026-09-03 | Updated ELECTION_DATA_SOURCES.md (new BC Municipal LECFA source section, verified live; Ontario/Manitoba Municipal elections and US Governor seats added to the live `elections` table — seats only, no candidate source yet; new §12 "Lessons learned" entry on a real CSDTYPE→entity-type cross-province code collision found while scoping Saskatchewan/PEI/NWT — misclassifies ~39 real municipalities as non-electable or leaves them unverified), adding-us-2026-midterm-candidates.md (Governor seats added onto the existing 2026 US Midterm Elections row; `start_us_2026_midterms.py` cannot be extended for Governor candidates — FEC has no Governor data) | Claude |
| 2026-08-24 | Updated VIRTUAL_INTERVIEW_SYSTEM.md (new "Post-launch refinements" section: unified `PitchPlayer.tsx` video-pitch player replacing two near-duplicate components, new `PitchPostPlayer.tsx` for standalone wall/feed pitch posts, share deep-links via `?pitch=<id>`, resolved the "feed-visibility scope of answer_pitch posts" open question — individually on the owner's wall, grouped in the general Feed — plus the `post_boundaries`/PGRST201/nonexistent-column/"Ghost-Unknown"/infinite-like bugs found fixing it, submit/approval status UI now matches the RPC's actual auto-approve-on-submit behavior), PLATFORM_SPEC.md §3C (spotlighted replies now show the owner's real name, not just a badge over their Ghost pseudonym — scoped to the wall/candidacy owner's own replies only, everyone else stays anonymized) | Claude |
| 2026-08-19 | Updated OUTREACH_GUIDE.md (§2 wall-slug field now editable in the admin UI, §4c layout, §4d delivery fix, renumbered legacy plain-text section to §4e), email-templates/README.md (points to `/admin/campaign` as the real send path, removed references to the broken `scripts/send-professional-emails.js` which is deleted), CAMPAIGN_AUTOMATION_SUMMARY.md / POLITICIAN_INVITES_GUIDE.md / SEND_INVITES_QUICKREF.md (banners pointing to the new admin tool — the legacy CLI script sends a different generic template, not the Mayor/Councillor HTML designs) — reflects the new `/admin/campaign` outreach flow (Mayor/Councillor HTML templates, search-to-add, editable wall slug) and the `send-email` MIME/SMTP delivery fix | Claude |
