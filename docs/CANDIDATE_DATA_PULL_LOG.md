...
## SD36 (Surrey) School Trustee was actually two separate elections, not one — 2026-09-13

**User caught it**, from the live seat page itself: White Rock has its own Mayor/Councillor election (correctly modeled), but its School Trustee page showed the exact same ~27-candidate list as Surrey's, and the user asked why — then, after checking White Rock's own official page and seeing only 3 names there, asked the sharper follow-up: do White Rock voters' real ballots show only those 3, or the full ~27?

**First answer given was wrong.** The initial assumption — SD36 is one school district, so it must be one shared at-large race, and White Rock's 3 names are just "whoever happened to file through White Rock's city hall for the same seat" — sounded right and matched every other multi-municipality case checked so far in this doc, but was never verified against an authoritative source before being stated. It was wrong.

**Correct structure, confirmed via Surrey Schools' own official elections page** (`surreyschools.ca/elections`, not a search snippet): *"The Board of Education of School District 36 is comprised of six trustees elected from the City of Surrey and one trustee elected from the City of White Rock."* Two separate Notices of Nomination are linked from that same page — one for each city. **White Rock voters genuinely only ever see their own 3 candidates for their 1 seat** — they do not vote in Surrey's 6-seat race at all, and vice versa. This is a real, different structure from the at-large SD36-wide model this doc had assumed.

**Fix**: created a new `election_seats` row (`role_title = 'School Trustee'`, `map_shape_id` = White Rock's existing Municipal shape — already had one from its own Mayor/Councillor seats, no new `map_shapes` row needed) and moved the 3 White-Rock-filed candidates (Dee Reiter, Phillip Warner, Laura Lattanzio — all 3 already correctly present in the DB, just attached to the wrong seat) onto it. Surrey's remaining 25 stay on the original SD36 seat.

**Bonus fix caught while verifying this**: a real duplicate found in the same list — "Afzalur Rahman" (correct spelling, has a photo, matches his own surrey.ca profile page) and "Afzular Rahman" (letters transposed, from the CivicInfo BC pass, no photo) were the same person. Zero engagement on either, merged the same way as every prior duplicate fix in this doc (COALESCE the data onto the richer profile, delete the thinner one).

**Open question, not yet checked**: is Surrey/White Rock a one-off, or do other BC municipalities sharing a school district with a bigger neighbor have the same "separate sub-district seats" structure rather than one shared at-large race? Given this was wrong on the first (unverified) assumption, **every other multi-municipality school district in this system should be treated as unverified until checked against that district's own official elections page**, the same way this one was — not assumed to follow whichever pattern happened to be checked most recently. Not done in this pass; flagged for a follow-up sweep.

**Follow-up sweep done the same day — the answer is "not a one-off at all."** Checked every BC school district CivicInfo BC's own data shows spanning more than one named area (via each candidate's `prevOffice` field) against `civicinfo_sept13.json` — **~40 of BC's 59 school districts** show multiple distinct sub-areas, not one shared list. Split into two real categories, verified per-district against that district's own official elections page before touching anything (same discipline as the Surrey correction above, not repeating the "assumed instead of verified" mistake):

- **Category A — real per-municipality split, using municipalities this system already has a `map_shapes` row for.** Confirmed and fixed 4 more the same day, each verified against that district's own official page, not a search snippet:
  - **SD44 – North Vancouver**: City (3 seats) / District (4 seats) — `sd44.ca/board-of-education/election`
  - **SD35 – Langley**: City (2 seats) / Township (5 seats)
  - **SD42 – Maple Ridge-Pitt Meadows**: Maple Ridge (5 seats) / Pitt Meadows (2 seats)
  - **SD63 – Saanich**: North Saanich (2) / Sidney (1) / Central Saanich (2) / Saanich (2) — `saanichschools.ca/board-education/trustee-elections`, which explicitly names the 4 municipalities administering nominations for each area (a genuine 5th sub-area, District of Highlands, is folded into Saanich's own area per that same page — not split out separately, since the official page itself doesn't treat it as a distinct administering municipality)

  Same mechanics as the Surrey fix: one new `election_seats` row per real sub-municipality (all 8 already existed as `map_shapes` rows from their own Mayor/Councillor elections — zero new boundary data needed), then every existing candidate's `election_candidates.seat_id` repointed from the old single district-wide seat onto the correct new one, matched by name against each candidate's CivicInfo BC `prevOffice` field (order-independent `same_person` match, same as every other name-reconciliation in this pipeline). **70 of 71 candidates matched cleanly**; the 1 exception (Joey Martin, SD63) has no `prevOffice` recoverable from any source checked (CivicInfo's current pull doesn't list him at all this pass — see the dropout tracking above — and LECFA carries no sub-area info at all) — left in place on the original SD63 seat rather than guessed at, flagged here rather than silently dropped.

- **Category B — real per-area split, but the areas are unincorporated regional-district territory with no `map_shapes` boundary at all.** Roughly 30 more districts (Cariboo-Chilcotin, Coast Mountains, Bulkley Valley, Sunshine Coast, Peace River North, Stikine, Vancouver Island North/West, Kootenay Lake, Kootenay-Columbia, Southeast Kootenay, Okanagan Skaha, Prince Rupert, Qualicum, and others) — **the same structural gap as the long-standing Regional District Electoral Area Director gap already flagged elsewhere in this doc**, not a new problem, just the same one showing up in school trustee races too. A further mixed group (Central Okanagan/Kelowna, Vernon, Kamloops/Thompson, Nicola-Similkameen, Prince George) has some seats that map to a real municipality (Kelowna, West Kelowna, Coldstream, Lumby, Mackenzie, Merritt) but whose actual electoral territory also extends into an adjacent regional-district area this system can't represent — so even those aren't a clean 1:1 fix the way Category A was. **Not attempted this pass** — would need real boundary-import work first, same prerequisite as the Regional District gap.

**Net result**: 5 BC school districts now correctly modeled as their real multiple elections (Surrey/White Rock, North Vancouver, Langley, Maple Ridge-Pitt Meadows, Saanich cluster) instead of one artificially-merged seat each. No candidates added or removed in this pass — pure re-attachment of existing candidacy rows to the correct seat, verified by re-counting every affected seat afterward against each district's own official page.

**Follow-up bug, caught by the user from "Find My District" itself**: the Surrey/White Rock split above only created a new seat for White Rock and left Surrey's 25 candidates on the *original* seat, which was still attached to the **School District**-type `map_shapes` row (the polygon covering both Surrey and White Rock) rather than Surrey's own **Municipal** row. "Find My District" matches an address against every boundary layer independently, so a White Rock address matched both White Rock's real seat *and* the leftover district-wide seat still carrying Surrey's 25 — a visible duplicate card. The other 4 districts split earlier the same day didn't have this bug (both sides got a real municipal seat from the start). **Fixed**: created a proper Surrey Municipal seat, moved the 25 onto it, then deleted the now-empty leftover School-District-type seats for Surrey, North Vancouver, Langley, and Maple Ridge-Pitt Meadows (checked first: 0 seat administrators, 0 other references on any of them). Saanich's leftover district-level seat was **not** deleted — it still carries Joey Martin, the one candidate from the original 71 that couldn't be matched to a specific sub-area — left in place rather than force-placed into one of the 4 real Saanich-cluster seats without evidence.

**Also removed, same conversation**: **59 "Board Chair" seats**, one pre-existing per BC school district, all with 0 candidates and 0 admins. Not a real BC election — school board chairs are elected internally by the trustees themselves after they take office, not voted on directly by the public — this was a modeling artifact from however seats got originally pre-created for BC school districts, unrelated to the trustee-election-splitting work above but caught in the same pass via the same "Find My District" card list.
