...
## Third bug found and fixed, a follow-up session (2026-08-18): duplicate profiles for sitting officeholders

**Read this too before touching `build_seat_and_candidates_sql()`.** A candidate row's `WHERE NOT EXISTS` guard (see bug #1 below) only checks whether *this script* already inserted a candidate with this name for this seat — it never checked whether the person already had a profile from the **officeholder import pipeline** (a completely separate script/data source: office holders get scraped and linked via `office_holders.linked_profile_id`, independent of anything in this file). A sitting member of Congress running for re-election in 2026 hit both pipelines: one profile from the officeholder import, a second independent one minted here when the FEC sync added them as a 2026 candidate.

This wasn't hypothetical — it happened for **179 real candidates**, confirmed and fixed as a one-time data migration (`supabase/migrations/20260818000005_merge_officeholder_candidate_duplicate_profiles.sql`, using the app's existing officeholder-wall-claim merge machinery so the fix got the redirect/audit-trail handling for free rather than reimplementing it). One of the 179 — Rick Larsen (WA-02) — had already caused a real, live bug: `CandidacyWall`'s "View Politician Wall" link computed a slug from name+role instead of using the real stored `wall_slug`, and that computed slug happened to collide with the *other*, unsupported duplicate profile's actual slug, silently routing every visitor to the wrong wall. Fixed separately in `CandidacyWall.tsx`/`ElectionSeatPageClient.tsx` (`resolvePoliticianWallSlug()` now prefers the real stored slug).

**Detection signal, if you need to re-check for this class of duplicate**: don't group by name alone — verified unsafe (grouping 31,685 politician profiles by name+country alone produces 547 groups, including two genuinely different real people both named "Adam Smith" holding different offices in different districts). The safe signal is structural: same `full_name` **and** the same underlying `map_shape_id`, with one profile reached via `office_holders.linked_profile_id` and a *different* profile reached via `election_candidates.politician_id` → `election_seats.map_shape_id` for that identical shape.

**Root cause prevention, fixed here**: `build_seat_and_candidates_sql()` now calls `find_existing_officeholder_profile(db_url, map_shape_id, name)` per candidate before deciding whether to mint a new stub — if a sitting officeholder profile already exists for this exact seat with a matching name, the new `election_candidates` row links straight to it instead of creating a second profile. No equivalent check exists yet in the Canadian sync scripts (`sync_federal_candidates.py`, `sync_ontario_candidates.py`, etc.) because none of them create live `profiles`/`election_candidates` rows at all — they only populate raw reference tables (`federal_election_candidates`, `provincial_election_candidates`); there's no Canadian equivalent of this script's "promote to a live candidate profile" step yet. If one gets built, it needs the same check from day one.
